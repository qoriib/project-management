 ...
 * Items in Requirements are displayed first, followed by Unplanned items.
 * Each item_id is aggregated exactly once to prevent duplicate rows.
 */
export async function getRequirementReport(
  ...
  try {
    ...
    // 1. Fetch Requirements grouped by item_id
    const requirementQuery = new QueryBuilder()
      ...
      .selectRaw("MAX(requirements.item_price_id) as item_price_id")
      ...
      .selectRaw(
        "COALESCE(SUM(requirements.qty * item_prices.price * (CASE WHEN requirements.has_tax = 1 THEN 1.12 ELSE 1.0 END)) / NULLIF(SUM(requirements.qty), 0), 0) as price",
      )
    ...
    // 2. Fetch Order Aggregates grouped by item_id
    ...
    orderQuery.groupBy("order_items.item_id");
    ...
    // 3. Fetch Receipt Aggregates grouped by item_id
    ...
    // 4. Build Lookups by item_id
    const orderMap = new Map<string, { total_ordered: number; total_order_price: number; avg_order_price: number }>();
    for (const row of orderAggregates) {
      orderMap.set(row.item_id, {
        avg_order_price: row.avg_order_price || 0,
    ...
    for (const row of receiptAggregates) {
      receiptMap.set(row.item_id, row.total_delivered || 0);
    }
    ...
    for (const req of requirements) {
      matchedItemIds.add(req.item_id);
      const orderInfo = orderMap.get(req.item_id);
      const delivered = receiptMap.get(req.item_id) || 0;
    ...
    const unplannedItemIds = Array.from(orderMap.keys()).filter((itemId) => !matchedItemIds.has(itemId));
    ...
    if (unplannedItemIds.length > 0) {
      const unplannedQuery = new QueryBuilder()
        ...
        .leftJoin("item_categories", "categories", "items.category_id = categories.category_id")
        .leftJoin("units", "units", "items.unit_id = units.unit_id")
        .where("items.item_id", "IN" as any, unplannedItemIds);
      ...
      for (const itemInfo of unplannedItemRows) {
        const orderInfo = orderMap.get(itemInfo.item_id);
        const delivered = receiptMap.get(itemInfo.item_id) || 0;
        ...
        unplannedList.push({
          ...
          planned_budget: 0,
          planned_volume: 0,
          price: orderInfo?.avg_order_price || 0,
          total_delivered: delivered,
          total_order_price: orderInfo?.total_order_price || 0,
          total_ordered: orderInfo?.total_ordered || 0,
          unit: itemInfo.unit ?? "-",
    ...
    // 7. Sort Planned Items by category and item name
    ...
    // 8. Sort Unplanned Items by category and item name
    unplannedList.sort((a, b) => {
      const catA = a.category ?? "LAINNYA";
      const catB = b.category ?? "LAINNYA";
      if (catA !== catB) return catA.localeCompare(catB);
      return (a.item_name ?? "").localeCompare(b.item_name ?? "");
