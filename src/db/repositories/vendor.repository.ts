...
class VendorRepository extends BaseRepository<Vendor, CreateVendor, UpdateVendor> {
  ...
  async findAllSorted(): Promise<VendorWithRelation[]> {
    const rows = await this.query("vendors")
      .select("vendors.*")
      .selectExists("SELECT 1 FROM order_items WHERE order_items.vendor_id = vendors.vendor_id")
      .orderBy("vendors.vendor_id", "ASC")
      .getMany<Vendor & { has_relation: number | boolean }>();
