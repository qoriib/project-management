...
export async function seedOrders(): Promise<void> {
  ...
  for (const order of orders) {
    const projectId = projMap.get(order.projectName),
      vendorId = vendMap.get(order.vendorName);
    ...
    if (!exists) {
      const dbItems = await Promise.all(
        ...
        timestamp = new Date(order.orderDate).getTime().toString().slice(-4),
        generatedOrderCode = `ORDER/${timestamp}`;

      await orderRepo.createWithItems(
        { order_code: generatedOrderCode, order_date: order.orderDate, project_id: projectId },
        dbItems,
      );
    }
  }
