...
| 6   | **Volume Diterima**   | Semua baris `0`               | Biarkan seluruh input Volume Diterima bernilai `0`, klik Simpan.               | Error Banner: _"Minimal ada 1 item yang diterima."_, data tidak tersimpan.                        |
| 7   | **Volume Diterima**   | Negatif (`-1`)                | Masukkan angka negatif pada kolom volume diterima.                             | Sistem menolak / mereset ke angka `0` atau menampilkan error validasi.                            |
| 8   | **Volume Diterima**   | Parsial ($0 < Qty < Sisa$)    | Contoh sisa order = `10`, masukkan volume diterima = `4`.                      | Valid, sistem mencatat penerimaan parsial 4 unit (sisa order menjadi 6 unit).                     |
| 9   | **Volume Diterima**   | Tepat Sisa ($Qty = Sisa$)     | Contoh sisa order = `10`, masukkan volume diterima = `10`.                     | Valid, sisa order menjadi `0` dan status item menjadi lunas terkirim penuh.                       |
| 10  | **Volume Diterima**   | Melebihi Sisa ($Qty > Sisa$)  | Contoh sisa order = `5`, masukkan volume diterima = `6`.                       | Error Tooltip: _"Melebihi sisa Order (5.00)."_, tombol Simpan mati (_disabled_).                  |
| 11  | **Volume Diterima**   | Desimal ($0.5$, $1.25$)       | Masukkan angka desimal pada material yang mendukung desimal (misal: kg/meter). | Valid, volume desimal tersimpan akurat dan mengurangi sisa order.                                 |
