...
|  5  | **Reset Database**    | Klik **Hapus Semua Data**        | Klik tombol **Hapus Semua Data**, baca dialog konfirmasi, lalu konfirmasi penghapusan. | Seluruh data pada database dibersihkan secara tuntas (clean wipe). Aplikasi kembali ke kondisi awal (tabel kosong). | Tombol ada, dialog konfirmasi ada — memanggil `resetDatabase()` ORM yang membersihkan seluruh tabel. | ✅     | `screenshots/settings-appearance.png` (halaman settings) |
